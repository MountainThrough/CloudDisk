...
public:
        ...
        : logger(Category::getRoot().getInstance("logger"))
    {
        ...
        // 2. 设置日志输出目的地
        OstreamAppender *ostreamApp = new OstreamAppender("console", &cout);
        ostreamApp->setLayout(ptn1);
        //@log 需要自己先创建文件，然后日志才能够写入,否则，不会写入文件
...
#define LogInfo(format, ...)                                                                       \
    {                                                                                              \
        ...
        Mylogger::getInstance().info(ss.str().append(std::string(format)).c_str(), ##__VA_ARGS__); \
    }
#define LogError(format, ...)                                                                       \
    {                                                                                               \
        stringstream ss;                                                                            \
        ss << "[" << __FILE__ << ":" << __FUNCTION__ << "():" << __LINE__ << "] ";                  \
        Mylogger::getInstance().error(ss.str().append(std::string(format)).c_str(), ##__VA_ARGS__); \
    }
#define LogWarn(format, ...)                                                                       \
    {                                                                                              \
        stringstream ss;                                                                           \
        ss << "[" << __FILE__ << ":" << __FUNCTION__ << "():" << __LINE__ << "] ";                 \
        Mylogger::getInstance().warn(ss.str().append(std::string(format)).c_str(), ##__VA_ARGS__); \
    }
#define LogDebug(format, ...)                                                                       \
    {                                                                                               \
        stringstream ss;                                                                            \
        ss << "[" << __FILE__ << ":" << __FUNCTION__ << "():" << __LINE__ << "] ";                  \
        Mylogger::getInstance().debug(ss.str().append(std::string(format)).c_str(), ##__VA_ARGS__); \
    }
