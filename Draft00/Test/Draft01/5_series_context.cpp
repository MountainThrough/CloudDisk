...
{
    cout << "I am callback1\n";
    //@info 测试结果就是序列严格按照顺序执行，先执行任务1和任务1的回调，再执行任务2
    sleep(10);
    // 在任务1的回调里面申请了堆上的内存
...
int main()
{
    ...
    WFRedisTask *redisTask3 = WFTaskFactory::create_redis_task(
        ...
        // nullptr
        redisCallback3);
    protocol::RedisRequest *req3 = redisTask3->get_req();
    req3->set_request("SET", {"58key", "value2"});
    // 创建一个序列
